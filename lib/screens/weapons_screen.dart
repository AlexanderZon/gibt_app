...
class WeaponsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<AccountWeaponData> list = [
      if (weaponsProvider.onDisplayWeapons.isNotEmpty &&
          charactersProvider.onDisplayCharacters.isNotEmpty)
        ...accountCharacters.map(
          (e) {
            List<Weapon> filteredWeapons = weaponsProvider.onDisplayWeapons
                .where((element) => element.id == e.weaponId)
                .toList();
            List<Character> filteredCharacters = charactersProvider
                .onDisplayCharacters
                .where((element) => element.id == e.characterId)
                .toList();
    ...
    list.sort((a, b) {
      if (a.weapon.rarity > b.weapon.rarity) {
        ...
        if (accountCharacterLevels.map[a.level]! >
            accountCharacterLevels.map[b.level]!) {
          return -1;
        } else if (accountCharacterLevels.map[a.level]! <
            accountCharacterLevels.map[b.level]!) {
          return 1;
        } else {
          return 0;
        ...
        body: GridView.count(
          ...
          children: list
              .map((e) => GestureDetector(
                    onTap: () => Navigator.pushNamed(context, 'character_form',
                        arguments: e.character),
                    child: WeaponCard(
                      data: e,
